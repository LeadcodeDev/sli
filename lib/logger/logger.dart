...
import 'package:sli/entities/folder_entity.dart';
import 'package:sli/io/control_character.dart';
import 'package:sli/io/key_stroke.dart';
import 'package:sli/io/ffi/terminal.dart';
import 'package:sli/logger/level.dart';
import 'package:sli/logger/link.dart';
import 'package:sli/io/terminal_overrides.dart';
...
abstract interface class LoggerContract {
  ...
  void clearComponentScreen();

  void write(String? message, {bool center = false});
  void writeln(String? message, {bool center = false});
  void info(String? message, {LogStyle? style, bool center = false });
  void err(String? message, {LogStyle? style, bool center = false});
  void alert(String? message, {LogStyle? style, bool center = false});
  void detail(String? message, {LogStyle? style, bool center = false});
  void warn(String? message, {String tag = 'WARN', LogStyle? style, bool center = false});
  void success(String? message, {LogStyle? style});
  void created(io.File file);
}
...
class Logger implements LoggerContract {
  ...
  final Terminal _terminal = TerminalOverrides.current?.createTerminal() ?? Terminal();

  KeyStroke Function() get _readKey {
    return () {
      _terminal.enableRawMode();
      final key = TerminalOverrides.current?.readKey() ?? KeyStroke.readKey.parse();
  ...
  void write(String? message, { bool center = false }) {
    ...
    if (center) {
      final width = io.stdout.terminalColumns < 60 ? io.stdout.terminalColumns : 60;
      final messageLength = message?.length ?? 0;
      final horizontalPadding = (width - messageLength) ~/ 2;
      final padding = ' ' * horizontalPadding;

      str = '$padding$message$padding';
    }
  ...
  @override
  void success(String? message, {LogStyle? style, bool center = false}) {
    if (level.index > Level.info.index) return;
    style ??= theme.success;
    _stdout.writeln(style(message));
  }
  ...
  void created(io.File file) {
    if (level.index > Level.info.index) return;
    _stdout.writeln('${green.wrap('created')} ${darkGray.wrap(link(uri: file.uri, message: file.path))}');
  }
  ...
  String prompt(String? message, {Object? defaultValue, bool hidden = false}) {
    ...
    final input = hidden ? _readLineHiddenSync() : _stdin.readLineSync()?.trim();
  ...
  List<String> promptAny(String? message, {String separator = ','}) {
    ...
    while (true) {
      ...
      final isEnterOrReturnKey =
          key.controlChar == ControlCharacter.ctrlJ || key.controlChar == ControlCharacter.ctrlM;
      final isDeleteOrBackspaceKey = key.controlChar == ControlCharacter.delete ||
  ...
  bool confirm(String? message, {bool defaultValue = false}) {
    ...
    final response =
        input == null || input.isEmpty ? defaultValue : input.toBoolean() ?? defaultValue;
    final lines = resolvedMessage.split('\n').length - 1;
    final prefix = lines > 1 ? '\x1b[A\u001B[2K\u001B[${lines}A' : '\x1b[A\u001B[2K';
    _stdout.writeln(
  ...
  List<T> chooseAny<T extends Object?>(
    ...
    final selections =
        hasDefaults ? defaultValues.map((value) => choices.indexOf(value)).toSet() : <int>{};
    var index = 0;
    ...
    void writeChoices() {
      ...
      for (final choice in choices) {
        ...
        if (isCurrent) {
          _stdout
            ..write(green.wrap('❯'))
            ..write(' ${lightCyan.wrap(resolvedDisplay(choice))}');
        } else {
          _stdout
            ..write(' ')
            ..write(' ${resolvedDisplay(choice)}');
        }
  ...
  Future<T?> navigate<T extends Object?>(
    ...
    T? defaultValue,
    String Function(T choice)? display,
    FutureOr Function()? onPrevious,
    FutureOr Function(T choice)? onNext,
    FutureOr Function(T value)? onCopy,
  }) async {
    final resolvedDisplay = display ?? (value) => switch (value) {
      FileEntity(key: final key) => key,
      FolderEntity(key: final name) => name,
      _ => value.toString(),
    };
    ...
    while (result == null) {
      final key = _readKey();
      final isArrowUpOrKey = key.controlChar == ControlCharacter.arrowUp;
      final isArrowDownOrKey = key.controlChar == ControlCharacter.arrowDown;
      final isArrowLeftOrKey = key.controlChar == ControlCharacter.arrowLeft;
      final isArrowRightOrKey = key.controlChar == ControlCharacter.arrowRight;
      final isReturnOrEnterOrSpaceKey = key.controlChar == ControlCharacter.ctrlJ ||
          key.controlChar == ControlCharacter.ctrlM ||
          key.char == ' ';

      if (key.char == 'c' && onCopy != null) {
        result = choices[index];
        isCopyAction = true;
        break;
      } else if (isArrowUpOrKey) {
        index = (index - 1) % (choices.length);
      } else if (isArrowDownOrKey) {
        index = (index + 1) % (choices.length);
      } else if (isArrowLeftOrKey) {
        clearComponentScreen();
        if (onPrevious != null) {
          await onPrevious();
        }
        break;
      } else if (isArrowRightOrKey) {
        if (choices[index] is! FileEntity) {
          clearComponentScreen();
